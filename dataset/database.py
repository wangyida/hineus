...
class GlossyRealDatabase(BaseDatabase):
    ...
    def _load_point_cloud(self, pcl_path, downsample=True):
        with open(pcl_path, "rb") as f:
            plydata = plyfile.PlyData.read(f)
            xyz = np.stack([np.array(plydata["vertex"][c]).astype(float) for c in ("x", "y", "z")], axis=1)
            if downsample:
                ply_o3d = o3d.geometry.PointCloud()
                ply_o3d.points = o3d.utility.Vector3dVector(xyz)
                ply_o3d = ply_o3d.voxel_down_sample(voxel_size=0.01)
                xyz = np.array(ply_o3d.points)
        return xyz
...
class NeRFSyntheticDatabase(BaseDatabase):
    def __init__(self, database_name, dataset_dir, testskip=8, scale_factor=.5):
        super().__init__(database_name)
        model_name = database_name.split('/')[-1]
        RENDER_ROOT = dataset_dir
        self.root = f'{RENDER_ROOT}/{model_name}'
