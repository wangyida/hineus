...
def texture_mapper(path='n2m_test', h0=1024, w0=1024):
    ...
    def _export_obj(v, f, h0, w0, ssaa=1, cas=0):
        ...
        rast, _ = dr.rasterize(glctx, uv.unsqueeze(0), ft,
                               (h, w))  # [1, h, w, 4]
        xyzs, _ = dr.interpolate(v.unsqueeze(0), rast, f)  # [1, h, w, 3]
        mask, _ = dr.interpolate(
            torch.ones_like(v[:, :1]).unsqueeze(0), rast, f)  # [1, h, w, 1]
        ...
        feats[tuple(inpaint_coords.T)] = feats[tuple(
        ...
        if ssaa > 1:
            feats0 = cv2.resize(feats0, (w0, h0),
                                interpolation=cv2.INTER_LINEAR)
            feats1 = cv2.resize(feats1, (w0, h0),
                                interpolation=cv2.INTER_LINEAR)
            feats2 = cv2.resize(feats2, (w0, h0),
                                interpolation=cv2.INTER_LINEAR)
